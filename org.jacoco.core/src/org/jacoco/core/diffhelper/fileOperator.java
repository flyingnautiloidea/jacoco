...
public class fileOperator {
    ...
    public String readStringFromFile(String filePath) {
        ...
        if (classesFile.isFile()) {
            ...
            return includesClassesString.trim() == "" ? null : includesClassesString.trim();
        } else {
            return null;
        }
    }
    ...
    String getArrayValue(String[] options, String key) {
        ...
        for (int inter = 0; inter < options.length; inter++) {
            ...
            if (currentEle.equals(key)) {
                /** params signals : -- */
                if ((inter + 1 <= options.length - 1) && !options[inter + 1].contains("--")) {
    ...
    public String diffFileString(String[] options) {
        String diffArgsValue = getArrayValue(options,
                "--diffFile");
        String diffFileLPath = getArrayValue(options,
                "--diffFileL");
